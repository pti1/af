...
docker run -d -t -h reprepro.example.com --name reprepro --dns $(docker inspect -f '{{.NetworkSettings.IPAddress}}' dns) --dns-search example.com --link dns:dns --volumes-from dns  --volume /GIT/af/reprepro:/kiki pti1/reprepro:secondversion /opt/startup.sh --isproxy=false
